...
1. **Install the package:**
   ...
   pip install .
   ```
   For accurate token counting (uses the Gemma tokenizer instead of the ~4 chars/token fallback):
   ```bash
   pip install ".[tokenizer]"
   ```

2. **HuggingFace Authentication (optional, only with `[tokenizer]`):**
   ```bash
...
3. **Configure the workspace sandbox:**
   Run the `init` command inside the project directory you want the agent to work on:
   ```bash
   cd my-project/
   gemma-harness init
   ```
   This writes a `harness_config.json` with a default command whitelist. Edit it to add or remove permitted commands.

4. **Start Ollama:**
   ```bash
   ollama serve
   ```
   The harness auto-detects the model's context window size via the Ollama API, so any model tag works.

## Usage

Launch the agent from inside your project directory:
```bash
gemma-harness
```
...
```bash
gemma-harness --model gemma3:8b
gemma-harness --model gemma3:8b --ollama-url http://192.168.1.5:11434
```

To persist a full event log for debugging or replay:
```bash
gemma-harness --log session.jsonl
```
...
Install in editable mode with the dev extra, then run pytest:
```bash
